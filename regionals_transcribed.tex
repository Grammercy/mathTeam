...
\item In quadrilateral $ICTM$, $\angle ICT=90^\circ$ and $\angle CTM=115^\circ$. Also, $IC=8$, $CT=6$, and $TM=11$. Determine the numeric area of quadrilateral $ICTM$. Express your answer as a decimal rounded to the nearest thousandth.\par\textit{Solution: }$72.503$.
...
\item Let
\[
k=\left[\sum_{n=1}^{\infty}\left(\frac13\right)^n\right]^{-2}.
\]
...
\item Let $p(x)$ and $q(x)$ be quintic polynomial functions with integer coefficients such that
\[
...
\item $\triangle ABC$ is a right triangle with right angle at $C$, and $AB=11$, $BD=7$, and $DA=6$. Here $D$ lies on $\overline{AC}$, as shown in the diagram. Determine $BC$, rounded to the nearest thousandth.\par\textit{Solution: }$6.325$.
...
\item Let $\sin\theta=\frac12$, where $0\leq\theta<2\pi$ and $\theta$ is measured in radians. The sum of all possible values of $\theta$ is $k\pi$. Determine $k$.\par\textit{Solution: }$1$.
...
\item In the diagram, $A=(0,0)$, $B=(26,0)$, $C=(x,y)$, $AC=51$, and $BC=55$. Point $P$ is inside $\triangle ABC$, and $[PAB]:[PAC]:[PBC]=1:2:3$, where brackets denote area. Determine the ordered pair for $P$, with reduced fractional coordinates.\par\textit{Solution: }$\left(\frac{739}{78},\frac{110}{13}\right)$.
\end{enumerate}
...
\item Solve for all $x$ such that $\log_{100}(x+2)=\log_{10}x$.\par\textit{Solution: }$2$.
...
\item In the diagram, $A,B,C$ lie on a circle centered at $O$, with radius $8$; $C$ lies on the major arc $AB$, and $AB=6$. Determine $\angle C$, in degrees rounded to the nearest hundredth.\par\textit{Solution: }$22.02^\circ$.
...
\item David and Jim play pool. The probability that David wins exactly two games out of four equals the probability that he wins exactly three out of four. Determine the probability that Jim wins a given game, as a reduced fraction.\par\textit{Solution: }$\frac25$.

\item Herik measures the angle of inclination from level ground to the top of a vertical pole as $50^\circ$. He walks 10 feet farther from the pole along the same line, where the angle is $40^\circ$. If he walks along that line to a point 10 feet from the base of the pole, the angle is $k^\circ$. Determine $k$, rounded to the nearest hundredth.\par\textit{Solution: }$70.57^\circ$.
...
\item In the diagram, $D$ lies on $\overline{AB}$ and $\overline{CD}$ bisects $\angle ACB$; the two angles at $C$ are $\theta$, with $\cos\theta=\frac14$. The side labels are $a=BC$ and $b=CA$, and $\frac1a+\frac1b=\frac1{20}$. Determine the exact length $CD$.\par\textit{Solution: }$10$.
\end{enumerate}
...
\item The probability David wins exactly two games out of four equals the probability he wins exactly three games out of four. Find the probability that Jim wins any given game, as a reduced fraction.\par\textit{Solution: }$\frac25$.
...
\item In the diagram, $A,B,C$ lie on $\overline{FE}$, $\overline{FD}$, and $\overline{ED}$, respectively. If $EA=10$, $AF=5$, $BF=12$, $BD=22$, $CD=16$, and $CE=19$, find the area of $\triangle ABC$ as a reduced improper fraction.\par\textit{Solution: }$\frac{4812}{85}$.
...
\]
find the exact length $AC$.\par\textit{Solution: }$16$.
...
\item In the diagram, $\angle CAB=30^\circ$, $AB=12$, $AC=k\sqrt3$, and $BC=w$, where $k,w$ are positive integers and $w<164$. Find the sum of all possible distinct values of $w$.\par\textit{Solution: }$216$.
...
\item In the diagram, $\triangle DEF$ is equilateral, and $A,B,C$ lie on $\overline{EF}$, $\overline{DF}$, and $\overline{DE}$, respectively. If $EA\cong FB\cong DC$, the ratio of the area of $\triangle ABC$ to the area of $\triangle DEF$ is $53:128$, $EF=256$, and $AE<AF$, find $AF$, rounded to the nearest tenth.\par\textit{Solution: }$187.9$.
...
\item One transformation that produces the graph $y=2x^2+24x-1$ from $y=x^2$ is a vertical shift $k$ units downward. Find $|k|$.\par\textit{Solution: }$73$.
...
\item A farmer wants to fence a rectangular plot of land bounded on one side by a river. He has 220 feet of chain-link fence and will not fence the river side. Each of the two widths perpendicular to the river needs double-layer fencing. If the enclosed area is to be maximized, find the length of the field parallel to the river. Ignore the thickness of the double fencing.\par\textit{Solution: }$100$.
...
\item (Multiple choice.) Write A if the conditions determine a conic, B if they overdetermine a conic, or C if they underdetermine a conic. Ellipse with center $(0,0)$ and eccentricity $\frac23$.\par\textit{Solution: }C.
...
\item In $\triangle ABC$, the side lengths are as shown, and $\angle ABC=120^\circ$. Find the exact length of $\overline{AC}$. The diagram gives $AB=20$ and $BC=80$.\par\textit{Solution: }$20\sqrt{21}$.
...
\]
and the length of $\overline{AD}$ is an integer, find the largest possible value of $AD$.\par\textit{Solution: }$4$.
